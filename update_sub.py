...
RENAME_DJJC = {
    ...
    # Tokyo nodes (DJJC cập nhật mới - có thể có hoặc không có flag emoji)
    "日本东京1号":             "🇯🇵 JP Tokyo 01 - VPN Trinh Hg",
    "日本东京2号":             "🇯🇵 JP Tokyo 02 - VPN Trinh Hg",
    "日本东京3号":             "🇯🇵 JP Tokyo 03 - VPN Trinh Hg",
    "🇯🇵日本东京1号":          "🇯🇵 JP Tokyo 01 - VPN Trinh Hg",
    "🇯🇵日本东京2号":          "🇯🇵 JP Tokyo 02 - VPN Trinh Hg",
    "🇯🇵日本东京3号":          "🇯🇵 JP Tokyo 03 - VPN Trinh Hg",
}
...
# ── Rules đầy đủ theo từng nhà cung cấp (KHÔNG gộp chung) ───────────────────
# Liangxin: có IP-CIDR 1.1.1.1/32, 8.8.8.8/32 và DOMAIN cn.bing.com,DIRECT
# DJJC: không có 3 entries đó
LIANGXIN_RULES = [
...
def process_b64(raw_b64: str, is_liangxin: bool):
    ...
    for line in lines:
        ...
        if old_name:
            ...
            if new_name is None:
                # Chưa có trong dict — giữ tên gốc + suffix
                # Loại bỏ các ký tự info node gốc (tiếng Trung thuần tuý không có emoji cờ)
                if "VPN Trinh Hg" not in old_name:
                    new_name = old_name + " - VPN Trinh Hg"
                else:
                    new_name = old_name
                # Ghi log để biết node nào chưa có rename rule
